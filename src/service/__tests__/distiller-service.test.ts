...
describe("DistillerService", () => {
  ...
  describe("summary generation", () => {
    it("should generate summaries for conversation slices", async () => {
      ...
      (distillerService as unknown as { memoryParser: typeof mockParser }).memoryParser = mockParser;
  ...
  describe("job statistics", () => {
    it("should track created count", async () => {
      ...
      mockProvider.setMockSummary("Content 1", createTestSummary("sum-1", "Note 1", 0.8));
      mockProvider.setMockSummary("Content 2", createTestSummary("sum-2", "Note 2", 0.8));

      const job = await distillerService.distillMemoryFile("/test/memory.json", []);

      expect(job.status).toBe("completed");
      expect(job.createdCount + job.mergedCount + job.skippedCount).toBeGreaterThanOrEqual(0);
    });
